...
import { applyEngagementRecommendationAction, generateEngagementRecommendationAction, recordEngagementCommercialOutcomeAction, recordEngagementFeedbackAction, refreshEngagementAnalyticsAction } from "@/server/actions/awo";
...
function allHashtags(recommendation: EngagementRecommendation): string[] {
  return [...new Set(Object.values(recommendation.hashtags).flat())];
}
...
export function EngagementIntelligencePanel({ organisationId, draftId, currentDraftVersion, initialPlatform, initialRecommendation, initialLearningOverview, initialDraftBody, initialDraftHashtags, draftLocked, canWrite }: { organisationId: string; draftId: string; currentDraftVersion: number; initialPlatform: CampaignPlatform; initialRecommendation: EngagementRecommendation | null; initialLearningOverview: EngagementLearningOverview; initialDraftBody: string; initialDraftHashtags: string[]; draftLocked: boolean; canWrite: boolean }) {
  ...
  function confirmApplication() {
    ...
    startTransition(async () => {
      const result = await applyEngagementRecommendationAction({
        ...
        variant: pendingApplication.variant,
        captionSnapshot: pendingApplication.caption,
  ...
  function refreshAnalytics() {
    startTransition(async () => {
      const response = await refreshEngagementAnalyticsAction({
        organisationId,
        draftId,
        platform,
        objectiveType,
      ...
      toast.success(response.result.recorded ? `Recorded ${response.result.recorded} new analytics snapshot${response.result.recorded === 1 ? "" : "s"}.` : response.result.alreadyRecorded ? "Analytics are up to date; no duplicate snapshots were created." : "No new published-post metrics were available yet.");
    });
  }
  ...
  function recordOutcome() {
    startTransition(async () => {
      const response = await recordEngagementCommercialOutcomeAction({
        ...
        enquiries,
        bookings,
        revenueMinor: Math.round(revenuePounds * 100),
        currency: "GBP",
  ...
  const linkedInGuidance = recommendation?.platform === "linkedin" ? (recommendation.creativeGuidance.linkedinPersonalProfile ?? null) : null;
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-3">
          ...
          {recommendation ? <Badge tone={isStale ? "danger" : appliedToCurrentVersion ? "positive" : "warning"}>{isStale ? "Outdated" : appliedToCurrentVersion ? "Applied" : recommendation.dataBasis === "performance_informed" ? "Performance-informed" : "Brand-informed"}</Badge> : null}
        </div>
      ...
      <CardContent className="flex flex-col gap-4">
        <section className="grid gap-3 rounded-md border border-border p-3" aria-label="Learning status">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
            <p>
              <span className="block font-medium text-foreground">Selection</span>
              {appliedToCurrentVersion ? "Applied to current draft" : learningOverview.latestFeedback ? "Recorded, not current" : "Not applied"}
            </p>
            <p>
              <span className="block font-medium text-foreground">Learning progress</span>
              {learningOverview.performanceSummary.sampleSize}/{learningOverview.performanceSummary.minimumSampleSize} comparable 7-day posts
            </p>
            <p>
              <span className="block font-medium text-foreground">Last analytics sync</span>
              {displayTime(learningOverview.lastAnalyticsSyncAt)}
            </p>
            <p>
              <span className="block font-medium text-foreground">Next scheduled collection</span>
              {displayTime(learningOverview.nextScheduledCollectionAt)}
            </p>
          </div>
          <p className="text-[11px] text-muted-foreground">
            Checkpoints: 24h {learningOverview.checkpoints.hours24 ? "✓" : "—"} · 72h {learningOverview.checkpoints.hours72 ? "✓" : "—"} · 7d {learningOverview.checkpoints.days7 ? "✓" : "—"}
          </p>
          {lastCollectionResult?.failed ? (
            <p className="text-[11px] text-danger">
              Latest refresh could not collect {lastCollectionResult.failed} published post{lastCollectionResult.failed === 1 ? "" : "s"}. Retry once; if it persists, check the Blotato connection.
            </p>
          ) : null}
          {learningOverview.exclusions.length > 0 ? (
            <details className="text-[11px] text-warning">
              <summary className="cursor-pointer">Why posts are not comparable</summary>
              <ul className="mt-2 grid gap-1 pl-4">
                {learningOverview.exclusions.map((item) => (
                  <li className="list-disc" key={item.code}>
        ...
        <div className="grid gap-2">
          {draftInputAssessment.kind === "content_brief" ? (
            <div className="rounded-md border border-warning/30 bg-warning-soft p-3 text-[12px] text-warning" role="alert">
          ...
          <Select aria-label="Commercial intent" value={commercialIntent} onChange={(event) => setCommercialIntent(event.target.value as CommercialIntent)} disabled={!canWrite || pending}>
            <option value="convert">Convert</option>
            <option value="engage">Engage</option>
            <option value="build_trust">Build trust</option>
          </Select>
          <Select aria-label="Cultural treatment" value={culturalVoiceLevel} onChange={(event) => setCulturalVoiceLevel(event.target.value as CulturalVoiceLevel)} disabled={!canWrite || pending}>
            <option value="neutral">Neutral</option><option value="conversational">Conversational</option><option value="light_naija">Light Naija — only when context supports it</option>
          </Select>
        ...
        {pendingApplication && recommendation ? (
          <section className="grid gap-3 rounded-md border border-warning/40 bg-warning-soft p-3" aria-label="Confirm recommendation application">
            ...
            <div className="grid gap-2 text-[11px]">
              <div>
                ...
                <p className="line-clamp-3 whitespace-pre-wrap text-muted-foreground">{initialDraftBody}</p>
              </div>
              <div>
                <p className="font-medium">Replacement caption</p>
                <p className="line-clamp-3 whitespace-pre-wrap text-muted-foreground">{pendingApplication.caption}</p>
              </div>
            </div>
            <p className="text-[11px] text-muted-foreground">
              Current hashtags: {initialDraftHashtags.length} · Replacement hashtags: {hashtags.length}. This creates a new draft version; approval remains mandatory.
            </p>
            <div className="flex gap-2">
              <Button type="button" size="sm" onClick={confirmApplication} disabled={pending}>
                <Check className="size-3.5" aria-hidden />
                {recommendation.platform === "linkedin" ? "Apply caption without hashtags" : "Apply caption + hashtags"}
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => setPendingApplication(null)} disabled={pending}>
                Cancel
              </Button>
            </div>
          </section>
        ...
        {recommendation ? (
          <div className="grid gap-3 border-t border-border pt-4" aria-live="polite">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="text-right text-[12px]">
                <div className="font-medium">Brand fit {recommendation.confidence}%</div>
                <div className="text-muted-foreground">Performance confidence {learningOverview.performanceSummary.performanceConfidence === null ? "— not enough data" : `${learningOverview.performanceSummary.performanceConfidence}%`}</div>
              </div>
            </div>
            ...
            {linkedInGuidance ? (
              <details className="rounded-md border border-border px-3 py-2 text-[12px]" open>
                ...
                {linkedInAuditRequired ? (
                  <div className="mt-3 rounded-md border border-warning/30 bg-warning-soft p-3 text-warning">This recommendation predates independent grounding. Generate a new recommendation before applying it.</div>
                ) : (
                  <div className="mt-3 grid gap-3 text-muted-foreground">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge tone="muted">Personal profile</Badge>
                      <Badge tone="muted">{LINKEDIN_ARCHETYPE_LABELS[linkedInGuidance.postArchetype]}</Badge>
                    </div>
                    <p className="text-[11px]">Editorial readiness only—not predicted reach or engagement.</p>
                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(linkedInGuidance.dimensions).map(([key, score]) => (
                        <div key={key} className="rounded border border-border p-2">
                          <span className="block text-[10px] text-subtle-foreground">{LINKEDIN_DIMENSION_LABELS[key as keyof typeof LINKEDIN_DIMENSION_LABELS]}</span>
                          <span className="font-medium text-foreground">{score}/5</span>
                        </div>
                      ))}
                    </div>
                    <div>
                      <p className="font-medium text-foreground">Reader value</p>
                      <p>{linkedInGuidance.audiencePromise}</p>
                    </div>
                    <div>
                      <p className="font-medium text-foreground">Credibility anchor</p>
                      <p>{linkedInGuidance.credibilityAnchor}</p>
                    </div>
                    <div>
                      <p className="font-medium text-foreground">Conversation prompt</p>
                      <p>{linkedInGuidance.conversationPrompt}</p>
                    </div>
                    <div>
                      <p className="font-medium text-foreground">Improve before publishing</p>
                      {linkedInGuidance.improvementActions.length > 0 ? (
                        <ul className="mt-1 grid gap-1 pl-4">
                          {linkedInGuidance.improvementActions.map((action) => (
                        ...
                        <p>No blocking editorial changes identified.</p>
                      )}
                    </div>
                    {linkedInGuidance.auditStatus === "passed" ? (
                      <p className="text-[11px] text-positive">
                        Independent grounding audit passed
                        {linkedInGuidance.auditAttempts === 2 ? " after one automatic repair" : ""}.
                      </p>
                    ) : null}
                  </div>
                )}
            ...
            <details className="rounded-md border border-border px-3 py-2 text-[12px]" open>
              ...
              <div className="mt-3 grid gap-2">
                <p className="whitespace-pre-wrap text-muted-foreground">{recommendation.recommendedCaption}</p>
                <p className="text-[11px] text-muted-foreground">{recommendation.platform === "linkedin" && hashtags.length === 0 ? "LinkedIn personal-profile mode applies clean, keyword-rich copy without hashtags." : `Applying this recommendation also replaces the draft hashtags with the ${hashtags.length} suggested hashtag${hashtags.length === 1 ? "" : "s"} shown below.`}</p>
                <div className="flex flex-wrap gap-2">
                  <Button type="button" variant="ghost" size="sm" onClick={() => copyText(recommendation.recommendedCaption, "Caption")}>
            ...
            {recommendation.alternativeCaptions.length > 0 ? (
              <details className="rounded-md border border-border px-3 py-2 text-[12px]">
                <summary className="cursor-pointer font-semibold">Alternative captions</summary>
                <div className="mt-3 grid gap-3">
                  {recommendation.alternativeCaptions.map((caption, index) => (
                    <div key={`${index}-${caption.slice(0, 20)}`} className="grid gap-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setPendingApplication({
                            variant: index === 0 ? "alternative_1" : "alternative_2",
                            caption,
                          })
                        }
                        disabled={!canWrite || pending || isStale || draftLocked || linkedInApplyBlocked || distributionGateBlocked}
            ...
            {recommendation.platform === "linkedin" ? (
              <div className="rounded-md border border-border p-3 text-[11px] text-muted-foreground">To use different LinkedIn wording, edit and save the draft first, then generate a new recommendation so the exact caption receives an independent audit.</div>
            ) : (
              <details className="rounded-md border border-border px-3 py-2 text-[12px]">
            ...
            <details className="rounded-md border border-border px-3 py-2 text-[12px]">
              <summary className="cursor-pointer font-semibold">Creative guidance, hashtags and reasoning</summary>
              <div className="mt-3 grid gap-4 text-muted-foreground">
                ...
                <div>
                  ...
                  <p>
                    {recommendation.creativeGuidance.visualHook} {recommendation.creativeGuidance.formatRecommendation}
                  </p>
                </div>
                <div>
                  <p className="font-medium text-foreground">Suggested hashtags</p>
                  {hashtags.length > 0 ? (
                    <div className="mt-1 flex flex-wrap gap-1.5">
                      {hashtags.map((hashtag) => (
                        <Badge key={hashtag.toLowerCase()} tone="muted">
                          {hashtag}
                        </Badge>
                      ))}
                    </div>
                  ) : (
                    <p className="mt-1">No hashtags recommended.</p>
                  )}
                  <p className="mt-2 text-[11px]">{hashtags.length > 0 ? "These replace the draft hashtags when you confirm the recommendation." : "Confirming removes existing draft hashtags so the saved post matches this recommendation exactly."}</p>
                </div>
                <div>
                  <p className="font-medium text-foreground">Why AWO recommends this</p>
                  <p>{recommendation.rationale}</p>
                </div>
                <p className="text-warning">{recommendation.limitations[0]}</p>
              </div>
            </details>
          ...
          <p className="text-[12px] text-muted-foreground">No recommendation recorded for this draft yet. Results remain advisory and cannot publish content.</p>
        )}

        {learningOverview.latestDraftMetric ? (
          <details className="rounded-md border border-border px-3 py-2 text-[12px]">
            <summary className="cursor-pointer font-semibold">Record enquiries, bookings and revenue</summary>
            <div className="mt-3 grid gap-2">
              <div className="grid grid-cols-2 gap-2">
                <Input type="number" min={0} aria-label="Enquiries" value={enquiries} onChange={(event) => setEnquiries(Number(event.target.value))} />
                <Input type="number" min={0} aria-label="Bookings" value={bookings} onChange={(event) => setBookings(Number(event.target.value))} />
              </div>
              <Input type="number" min={0} step="0.01" aria-label="Revenue in pounds" value={revenuePounds} onChange={(event) => setRevenuePounds(Number(event.target.value))} />
              <Input aria-label="Outcome note" value={outcomeNote} onChange={(event) => setOutcomeNote(event.target.value)} maxLength={500} placeholder="Optional evidence note" />
              <Button type="button" size="sm" onClick={recordOutcome} disabled={!canWrite || pending || bookings > enquiries}>
                Record outcome snapshot
              </Button>
              <p className="text-[11px] text-muted-foreground">Values are append-only snapshots linked to the latest eligible published attempt. Bookings cannot exceed enquiries.</p>
            </div>
          </details>
        ) : null}
      </CardContent>
