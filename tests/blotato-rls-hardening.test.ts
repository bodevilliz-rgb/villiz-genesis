...
function baseJob(overrides: Partial<PublishingJob> = {}): PublishingJob {
  return {
    ...
    // This file's tests are fundamentally about the live/RLS-sensitive
    // account-resolution path (per-test blotatoConfig().livePublishingEnabled
    // overrides select live vs simulation) — executionMode:"live" is what
    // makes resolveEffectiveLivePublishing actually honour those per-test
